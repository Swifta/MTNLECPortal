 ...
 */
package com.swifta.lecportal.managedbeans;

import com.swifta.lecportal.entities.AuditTrail;
import com.swifta.lecportal.entities.SchoolAdmin;
import com.swifta.lecportal.entities.School;
import com.swifta.lecportal.entities.AdminRole;
...
import com.swifta.lecportal.entities.TransactionHistory;
import com.swifta.lecportal.entities.PartnerService;
import com.swifta.lecportal.datamodel.StudentDataModel;
import com.swifta.lecportal.datamodel.TransactionHistoryDataModel;
import com.swifta.lecportal.dblogic.AuditTrailDatabase;
import com.swifta.lecportal.dblogic.ExchangeRateDatabase;
import com.swifta.lecportal.dblogic.MinimumRateDatabase;
import com.swifta.lecportal.dblogic.PortalAdminDatabase;
import com.swifta.lecportal.dblogic.SchoolDatabase;
import com.swifta.lecportal.dblogic.StudentDatabase;
import com.swifta.lecportal.dblogic.TransactionHistoryDatabase;
...
import com.swifta.lecportal.utils.Month;
import com.swifta.lecportal.utils.NumberUtilities;
import com.swifta.lecportal.utils.PageUrls;
...
public class SchoolBean {
    ...
    private MinimumRateDatabase minimumRateDatabase = new MinimumRateDatabase();
    ...
    private Date firstDate = new Date(), secondDate = new Date(), txnFirstDate = new Date(), txnSecondDate = new Date(), minimumRateDate = new Date(),exchangeRateDate = new  Date();
    private AppValues appValues = null;
    private List<String> selectedActionsPerformed = null, selectedActionTypes = null;
    private String currentDate = "", fromDate = "", toDate = "", selectedAction = "", selectedActor = "", subTotal = "", searchParameter = "", schoolAdminFileName = "", schoolsFileName = "", adminListFileName = "", transactionHistoryFileName = "", registeredStudentsFileName = "",minimumRatesFileName="";
    ...
    private List<ExchangeRate> allExchange;
    private List<MinimumRate> allMinimumRates;
    ...
    public void createExchangeRate() {
        logger.info("Afer create exchange rate " + exchangeRate.toString());
        try {
            //hard coded ids....prone to fail if the id of the default payment methods fail.
            exchangeRate.setDatecreated(new Timestamp(new Date().getTime()).toString());

            if (!exchangeRateDatabase.createExchangeRate(exchangeRate)) {
                ...
                createAuditTrail(appValues.AUDITTRAIL_CREATE, appValues.AUDITTRAIL_EXCHANGERATE_ENTITY, portalAdmin, null);
                logger.info("--------------------------------after creating exchange rate");
                showMessage("New Rate created ... ");
                exchangeRate = new ExchangeRate();
            }
    ...
    public void createMinimumRate() {
        logger.info("Afer create minimum rate " + exchangeRate.toString());
        try {
            //hard coded ids....prone to fail if the id of the default payment methods fail.
        //    minimumRate.setDatecreated(new Timestamp(new Date().getTime()).toString());

            if (!minimumRateDatabase.createMinimumRate(minimumRate)) {
                PortalAdmin portalAdmin = (PortalAdmin) portalSession.getAppSession().getAttribute("logged_in_portal_admin");
                logger.info("=====================create audit trail==================" + portalAdmin);
                createAuditTrail(appValues.AUDITTRAIL_CREATE, appValues.AUDITTRAIL_MINIMUMRATE_ENTITY, portalAdmin, null);
                logger.info("--------------------------------after creating minimum rate");
                showMessage("New Minimum Rate created ... ");
                minimumRate = new MinimumRate();
            }
    ...
    public List<ExchangeRate> getAllExchange() {
        return allExchange;
    }

    public void setAllExchange(List<ExchangeRate> allExchange) {
        this.allExchange = allExchange;
    }

    public List<MinimumRate> getAllMinimumRates() {
        return allMinimumRates;
    }

    public void setAllMinimumRates(List<MinimumRate> allMinimumRates) {
        this.allMinimumRates = allMinimumRates;
    }

    public String getMinimumRatesFileName() {
        return minimumRatesFileName;
    }

    public void setMinimumRatesFileName(String minimumRatesFileName) {
        this.minimumRatesFileName = minimumRatesFileName;
    }
    ...
    public String getExchangeRatesFileName() {
        return "ExchangeRates_" + new Timestamp(new Date().getTime()).toString();
    }

    public void setExchangeRatesFileName(String schoolsFileName) {
    ...
    public Date getMinimumRateDate() {
        return minimumRateDate;
    }

    public void setMinimumRateDate(Date minimumRateDate) {
        this.minimumRateDate = minimumRateDate;
    }
    ...
    public void searchExchangeRates() {
        try {
            List<ExchangeRate> allExchangeRates = new ArrayList<ExchangeRate>();
            allExchangeRates = exchangeRateDatabase.getAllExchangeRates();
            this.allExchange = allExchangeRates;
            showMessage(allExchangeRates.size() + " exchange rate(s) retrieved");
        } catch (SQLException ex) {
            logger.error(ex);
            this.allExchange = new ArrayList<ExchangeRate>();
        }
    }
    public void searchMinimumRates() {
        try {
            List<MinimumRate> allMinimumRates = new ArrayList<MinimumRate>();
            allMinimumRates = minimumRateDatabase.getAllMinimumRates();
            this.allMinimumRates = allMinimumRates;
            showMessage(allMinimumRates.size() + " minimum rate(s) retrieved");
        } catch (SQLException ex) {
            logger.error(ex);
            this.allMinimumRates = new ArrayList<MinimumRate>();
        }
    }
